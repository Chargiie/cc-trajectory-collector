...
MARK_COL = 11      # K「已生产」= run 目录名(有值=已完成)
STATUS_COL = 12    # L「生产状态」= done / FAIL:reason
TIME_COL = 13      # M「完成时间」
...
def ensure_headers(ws):
    if not ws.cell(1, MARK_COL).value:
        ws.cell(1, MARK_COL).value = "已生产"
    if not ws.cell(1, STATUS_COL).value:
        ws.cell(1, STATUS_COL).value = "生产状态"
...
def worker(jobs_q, port, log_dir, args, xlsx, wb, ws, prog_path, stats):
    while True:
        ...
        try:
            ...
            status = f"done({res['reason']},{traj['meta']['num_main_calls']}步)"
            ...
            mark_xlsx(xlsx, wb, ws, row, run_dir, f"FAIL:{e}")
