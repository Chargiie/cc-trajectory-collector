...
def _token_usage(main):
    """汇总该轨迹主 loop 各响应的 token 用量(来自响应 usage 字段)。"""
    tin = tout = tcc = tcr = 0
    counted = 0
    for o in main:
        u = (o.get('response_reassembled') or {}).get('usage') or {}
        if not u:
            continue
        counted += 1
    ...
    return {
        "input_tokens": tin,                       # 新鲜(未命中缓存)输入
        "output_tokens": tout,                     # 生成
        "cache_creation_input_tokens": tcc,        # 写入缓存
        "cache_read_input_tokens": tcr,            # 命中缓存(便宜)
        "total_input_tokens": tin + tcc + tcr,     # 各轮上下文输入总量
        "total_tokens": tin + tcc + tcr + tout,    # 总吞吐(输入+输出)
        "calls_counted": counted,
    }
